// s14_cron_scheduler/Program.cs -- Cron Scheduler
//
// "Fire on schedule, no human kick needed" — a daemon thread polls
// every second, fires matching jobs, and pushes them into a queue the
// agent loop drains before each LLM call.
//
// Compared to s13:
//   + CronScheduler with durable persistence (.scheduled_tasks.json)
//   + 3 new tools: schedule_cron, list_crons, cancel_cron
//   + OnBeforeLlmCall hook drains the queue, appending fired jobs as
...
// Drain background tasks + cron queue before each LLM call
...
Console.WriteLine("s14: Cron Scheduler — durable, daemon-thread, queue-fed");
Console.WriteLine("Type a task. Try 'schedule a job for every minute'. Type q to quit.\n");
await Repl.RunAsync(agent, "\u001b[36ms14 >> \u001b[0m");
