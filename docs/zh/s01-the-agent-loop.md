...
3. 追加助手响应。检查 `StopReason` -- 如果模型没有调用工具, 结束。
...
组装为一个完整函数 (`AgentCommon/Agent/AgentHarness.cs`):
...
| 组件          | 之前       | 之后                           |
|---------------|------------|--------------------------------|
| Agent loop    | (无)       | `while stop_reason == "tool_use"` |
| Tools         | (无)       | `bash` (单一工具)              |
| Messages      | (无)       | 累积式消息列表                 |
| Control flow  | (无)       | `StopReason != "tool_use"`     |
